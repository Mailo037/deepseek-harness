...
Composer 文件列表读取 `sessionStats.fileChanges`：从成功且已配对的结果元数据派生每个精确路径的紧凑累计计数。文件数与列表由同一投影维护，因此历史分页不会改变它们。状态版本 3 使仅保留路径集合的检查点失效。另建浏览器历史缓存会重复日志并要求加载旧 diff 正文；现有投影传输在打开会话时提供列表，并实时刷新。产出文件 Web 场景打开一个编辑位于已加载尾页之外的冷会话，并为完整文件列表记录快照。
