...
The composer file disclosure consumes `sessionStats.fileChanges`: compact cumulative counts per exact path derived from successful, paired result metadata. The same projection owns the file count and rows, so paging cannot change either. State version 3 invalidates checkpoints that retain only path membership. Keeping a second browser history cache would duplicate the log and require loading old diff bodies; the existing projection delivery supplies the list on open and refreshes it live. The produced-files Web scenario opens a cold session with edits outside the loaded tail and snapshots the complete file list.
