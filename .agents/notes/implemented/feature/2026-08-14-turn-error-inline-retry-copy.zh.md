...
- 重试会发送一条排队用户轮次，提示为本地化的续跑文案（`message.turnError.retryMessage`：“发生意外错误，请继续你之前的工作。” / "An unexpected error occurred, please continue what you were doing."）。它走普通发送路径 —— 会话作用域的 `conversation.send` —— 因此被发送的提示是一条持久的、可见的用户消息，后续每个轮次都能看到（[model-visible ⟺ logged](../../../../docs/architecture.zh.md)）。
