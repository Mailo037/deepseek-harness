...
- 选择 Trajectory 标签页会先提交居中的加载状态，再在下一动画帧物化计时概览和记录表，因此即使已加载窗口很长，活动视图也会在其投影工作开始前作出响应。
...
**在选择标签页时构建完整的 Trajectory 视图。** 不予采纳：很长的已加载历史会阻止选中的标签页绘制，使数据准备看起来像是忽略了点击。居中的状态会先于首次高开销投影提交。
...
选择 Trajectory 会在计时概览和记录表构建前确认该操作。
