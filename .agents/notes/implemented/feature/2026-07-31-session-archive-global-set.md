...
- Gateway cleanup: before the archive set commits, the gateway cancels the root agent and kills jobs owned by it and live subagents. It walks only the live Session store, then passes every live subagent to the core interruption primitive; one-shot ids are no-ops, so descriptor or persistence reads never delay the archive response.
