...
应用启动时，`AppUpdatePlugin` 会向 `https://api.github.com/repos/Mailo037/deepseek-harness/releases/latest` 发起一次尽力请求。它只接受已发布、非草稿、非预发布的 Release，标签必须严格为 `android-vMAJOR.MINOR.PATCH`，asset 必须严格为 `harness-remote-android-vMAJOR.MINOR.PATCH.apk`。`ReleaseVersion` 会比较全部三个数字部分，因此稳定更新不会被误认为预发布版本或降级。
...
Android `versionName` 来自 `apps/android/package.json`；`native/app.build.gradle` 保留初始 version code `1`。每个后续 Android Release 都会提升该 version code，同时保留签名证书。`scripts/sync-native.mjs` 会把原生源文件、资源、asset 与 JVM 测试复制到 Capacitor 生成的 Gradle 项目，Android workflow 调用同一个同步命令。
