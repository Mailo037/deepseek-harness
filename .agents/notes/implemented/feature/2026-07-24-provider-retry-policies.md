...
Both modes use exponential local delays from `initialDelayMs` to `maxDelayMs`. `jitterRatio` multiplies each target by a uniform sample in `[1 - jitterRatio, 1 + jitterRatio]`, then applies the cap. A `RATE_LIMIT` failure scales its local target by `rateLimitMultiplier` (default 2), so a rate-limited request waits longer before its next retry than a server or transport hiccup. A positive provider `Retry-After` within the cap remains exact, unjittered, and unscaled. An over-cap provider delay makes normal mode delegate; always mode retains its guarantee by using the configured local backoff.
