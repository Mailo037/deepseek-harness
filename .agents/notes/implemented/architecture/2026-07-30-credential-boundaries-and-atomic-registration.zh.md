...
**存下的凭据不是针对模型的进程边界，而 README 就是这么写的。**没有任何一个表面会把文档提升进 `process.env`，模型也从不会拿到其解析后路径。后续的 [Windows DPAPI 静态存储决策](2026-08-30-windows-dpapi-credential-at-rest-protection.zh.md)让 Windows 上的直接文件读取与复制文件不再泄露凭据名称或值，同时保留精确限制：同用户代码仍可调用 DPAPI。POSIX 仍是仅属主可读的明文。必须对已经以部署帐户执行的任意代码隔离凭据时，需要独立的最小权限身份或 broker。
...
- **在本次变更中交付平台静态存储保护**——这里选择延后，以控制请求一致性与写入完整性修复的范围。后续的 [Windows DPAPI 决策](2026-08-30-windows-dpapi-credential-at-rest-protection.zh.md)加入经过认证的当前用户加密，但不宣称同用户进程隔离；原生 macOS 与 Linux 存储仍是暂缓事项。
...
与 `update()` 相关的行为多了成文的失败模式：凭据写入现在可能因锁截止时间到期、或磁盘文档无法解析而失败，`describe()` 对它不会改写的多行条目报告 `writable: false`。`LlmAdapter` 的注册方无需改动即可继续工作（句柄本身仍可当作释放器调用），`DeepSeekConnectionOptions` 则新增了凭据字段，因此以编程方式构造该适配器必须提供 `apiKeyEnv`。原生 macOS 与 Linux secret store，以及逐值修订号检查仍是暂缓事项；同一引用的写入仍采用后写胜出。
