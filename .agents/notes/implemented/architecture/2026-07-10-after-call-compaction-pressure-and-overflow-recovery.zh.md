...
当适配器公布的容量偏乐观时（默认 DeepSeek 目录宣称的窗口远大于部署实际执行的上限），提供方已确认的溢出也会让该聊天学到它的真实窗口。恢复通过 `ctx.tokenMeter` 计量被拒绝的请求，并把该大小记录为该聊天的有效上下文窗口，即使不可分表层或重试上限耗尽阻止了重试也会记录。后续主动压力会用这个被采纳的窗口取代公布的容量，从而对真实上限作出反应，而不是一路冲向下一次溢出。
...
下一个 pre-step 的压力检查描述前一个已完成的路由请求，包括持久工具结果与新领取输入。可选的无模型剪枝会在选择摘要前移除可预测的工具输出体积，也能独立产生足以重试的进展。当成功 usage 锚点不存在时，规范化溢出提供兜底路径。恢复有明确上限、以取消为准，并保持单调：只有模型可见的表层 generation 变化后才重试，同时溢出也会把被拒绝请求的计量大小采纳为该聊天的有效窗口，使后续主动压力匹配真实上限。
