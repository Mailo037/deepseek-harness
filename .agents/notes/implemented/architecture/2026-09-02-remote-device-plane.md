...
- **Notification bridge** — the plugin subscribes to `session/event` and broadcasts a frame to every connected device when a turn ends with an error or completes (both configurable). The visible message uses the latest durable session title and falls back to the session id before a title exists. Offline devices miss frames; pull-on-reconnect is deferred.
