...
/** Deployment apps that produce platform artifacts instead of npm packages. */
const privateDeploymentAppDirectory = /^apps\/android$/
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/(?!android$)[^/]+|vendor\/[^/]+)$/
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  ...
  // The desktop app ships its emitted main tree and the shipped agent-preset
  // root beside it (both source and built layouts resolve them relative to
  // the package).
  '@deepseek-ai/dsh-electron': ['lib/types', 'config'],
}
...
function expectedDshPackageFiles(manifest: PackageManifest): readonly string[] {
  ...
  return [
    ...
    // A surface bundle's launch backend row is its own bundle for the same reason.
    ...exportDefault(manifest, './launch-item') === './lib/launch-item.js' ? ['lib/launch-item.js'] : [],
    ...extras,
...
export function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  } else if (!experimentalPackageDirectory.test(dir)
    && !privateDeploymentAppDirectory.test(dir)
