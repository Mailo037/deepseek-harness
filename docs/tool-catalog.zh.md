...
| `@deepseek-ai/dsh-tool-session-query` | `session_event_read`、`session_event_search`、`session_event_trace`、`session_search`、`session_summary`、`session_trace` | `ctx.tools`、`ctx.systemPrompt`、`ctx.sessionQuery`、`a calling Agent for workspace authority` | `tool/call`、`tool/result` | - | 这 6 个只读工具会隐藏提供方游标，根据不可变的调用 agent 会话为每个结果授权，删节识别出的凭据文本，并限制完整的模型可见输出。该包需要选择启用；通用超时或 spill 策略可以施加更严格的截止时间或展示策略。 |
...
从一个已获授权的会话中读取一个目标事件，以及可选的相邻原始事件概述。
...
将已获授权的 Session 读取为有界、以事件为依据的摘要，其中包括其目标、明确决策、观察到的已更改文件和未完成待办。
...
这 6 个只读工具会隐藏提供方游标，根据不可变的调用 agent 会话为每个结果授权，删节识别出的凭据文本，并限制完整的模型可见输出。该包需要选择启用；通用超时或 spill 策略可以施加更严格的截止时间或展示策略。
