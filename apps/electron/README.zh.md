...
- 主进程提供**启动后端**（`launchSettings` 服务）：General 设置中的「开机自启」开关通过 `app.setLoginItemSettings` 应用——Windows 注册表 Run 键始终可用，macOS 仅限已打包应用，Linux 因 Electron 不提供登录项 API 而不受支持。
...
- **登录项以偏好为准，不做回读**：设置开关保存偏好，主进程在下一次启动时和每次变更时应用它；当偏好如此声明时，在应用外（任务管理器、系统设置）删除的条目会在下次应用启动时重新写回。
