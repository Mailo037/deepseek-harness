...
served page 可在客户端启动前设置 `__DSH_REQUEST_AUTH__.query`。浏览器载体会将这些参数复制到 unary HTTP 请求、Typert Remote 请求和两条 WebSocket 下行链路；当 WebView 抑制第三方 cookie 时，认证 iframe 客户端仍可显式携带 token。

嵌入外壳可通过 `postMessage` 发送 `{ type: "dsh/client-shell-context", version: 1, shell: "android" }`；`ctx.connection.shell` 会向展示插件发布这一信息性上下文。接受后，连接服务会把当前状态及之后每次切换以 `{ type: "dsh/client-connection-state", version: 1, state }` 发送给父页面，让 Android 外壳在 iframe 外部独占一个连接指示器。两类消息都只携带展示状态，绝不替代请求认证：它们可以调整布局和重连呈现，但不能解锁 API 方法或认证发送方。
