...
export interface WebBlockLabels {
  /** Placeholder when a search card carries no answer and no sources. */
  emptySearch: string
  /** Note under a search card whose source list was capped by the tool. */
  sourcesTruncated: string
  /** Note beside a fetch card whose content was capped by the tool or the output budget. */
...
function WebSearchBlock({ answer, sources, truncated, className, labels }: WebSearchBlockProps) {
  ...
  const copy: WebBlockLabels = { ...DEFAULT_LABELS, ...labels }
  return (
    <div className={clsx(css.block, className)} data-web="search">
      ...
      {empty ? (
        <div className={css.empty}>{copy.emptySearch}</div>
      ) : (
