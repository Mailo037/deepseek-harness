...
`ElevatorLabel` 把单行选择标签保留在裁切轨道内。首个值保持静止；替换值会在 260ms 内从上方进入，前一个值则从下方离开。它为每次替换提供新的动画 key，并在同一时长内插值轨道宽度，因此较短或较长的值不会让外围 pill 跳变；`prefers-reduced-motion: reduce` 会直接替换文本和尺寸而不产生运动。
