  ...
  it('gives every section a nav glyph, distinct for the ids the shell knows', () => {
    mount({
      rows: [
        ...
        { id: 'remote', order: 40, label: 'Remote' },
        { id: 'fs-deny', order: 50, label: 'Access restrictions' },
        { id: 'about', order: 60, label: 'About' },
        { id: 'contributed', order: 70, label: 'Contributed' },
      ],
    })
    openPanel()
    // Glyphs carry no id of their own, so the drawn paths are what tells them apart.
    const glyphs = ['General', 'Models', 'Agent presets', 'Plugins', 'Remote', 'Access restrictions', 'About', 'Contributed']
    ...
    // The seven ids the shell names get their own glyph; every other section —
