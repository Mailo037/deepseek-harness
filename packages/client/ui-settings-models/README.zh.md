...
提供方行支持用户拖拽排序：每行行首的拖拽手柄支持鼠标拖放或 ArrowUp/ArrowDown 键盘操作，拖放时以箭头插入标记提示落点位置，排列顺序会以 `models.providerOrder` 偏好（提供方路由 ID，第一名 = 最前）持久化到共享设置文档中。Host api-proxy 会将该偏好应用到 `llm.providers` 和模型选择器目录组 (`session.models`/`llm.models`) 中，因此用户在此处设定的顺序即是模型选择器展示的先后顺序；没有存储位置的提供方会排在最后，保持其自然顺序。首次运行时的设置卡不是拖拽目标，只读设置文档会禁用所有手柄。
