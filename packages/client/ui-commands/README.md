...
The `/` source also implements the slash pipeline's lexicon hooks: `lexicon` answers the hot roll of resolvable host commands plus currently-available contributions, each entry carrying the `command` token domain so the composer's plain-token scan renders `/name` as a glyphed capsule; `subscribeLexicon` rides the directory's settlement listeners, so warm, soft invalidation, preset switches, and reconnect resets republish the roll without a fetch from the render path.
