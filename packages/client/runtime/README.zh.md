...
`ConversationSnapshot.historyTurns` 保留 Host 提供的、被历史分页截断的已完成轮次时间。当前缀分页使 `startSeq` 进入连续事件窗口时，该条目标记为已加载，并继续保留以维持界面分组。重连会丢弃旧条目并拒绝过期的历史页响应。
...
`WorkspaceListState.pinnedSessionIds` 镜像 Host 的注册表级全局固定顺序。`workspace.list` 基线、`setSessionPinned` 一元回声和 `host/pinned-sessions-changed` 帧各自安装完整快照；列表刷新期间到达的固定变更会取代过期基线。`WorkspaceRuntime.setSessionPinned(sessionId, pinned)` 仅在 Host 接受持久成员变更后才完成。归档会一起安装返回的归档与固定快照，因此客户端不会把同一会话同时投影为已归档和已固定。
...
`ConversationSnapshot.queue` 是 Host 提供的 `agent.inbox.nextTurn` 权威瞬态快照；待处理的 next-step steering（中途引导）不进入此投影。每行携带其 `MessageId`、所有内容块均为文本时的完整可编辑文本，以及扁平化预览。Host 根据持久 `agent/inbox/spliced` 变更派生完整 `session/queue` 快照，并在重连时发送基线；面向单条消息的 `agent/inbox/inserted`、`claimed` 与 `discarded` 通知不用于重建该投影。`Session.updateQueue()` 经 Host 侧 `Inbox.splice()` 发送编辑／移除／重排（`move`，目标索引由 Host 侧收敛）操作，客户端不做乐观变更，因此下一份 Host 快照是唯一可见的提交结果，claim 竞态则可能呈现 `queue-item-not-found`。
...
历史读取对传输抛错（不重试 Host 已应答的业务失败）做两次短退避重试后才进入错误状态。载体对每次 unary 读取设有截止时间，而冷启动或超大日志可能让 Host 在一个截止时间内无法完成分页准备，且客户端中止并不会取消该服务端工作，因此重试通常落在已完成的准备结果上，而不是把一次缓慢读取变成死掉的会话。`ISession.reloadHistory()` 为残留错误状态提供重新执行 open 流程的入口。
