...
Unified Web `@file` and `@session` source. The browser starts the `fileReferences/list` and `sessionReferenceResolver/candidates` Remote calls together for an unquoted token, deterministically orders files before sessions, and renders the rows under non-selectable file and session section headings without a redundant raw `reference` source title. Each row leads with its domain glyph (folder, file, chat bubble) in place of a text prefix; either failed candidate domain degrades independently. An open `@"…` token searches files only.
