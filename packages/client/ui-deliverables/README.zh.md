...
如果收到的统计投影尚未包含逐文件明细，输入栏会暂时使用已加载轮次中的变更数据；完整统计到达后自动更新，缺失明细不会使输入栏崩溃。
...
`LineChangeSummary` 在 Composer 的 input dock 中读取整会话的 `sessionStats.fileChanges` 投影。首个历史页面提供按路径汇总的紧凑计数，工具完成时由投影更新刷新；无需加载较早消息或 diff 正文。同一路径的新增与删除行数累计。点击居中的总计会打开可滚动列表，展示文件名、完整路径与计数；按 Escape 或点击外部关闭。未装配投影的组合回退到已加载的轮次数据。该展示不会新增 Session event 或模型输入。
...
- **Composer 变更摘要依赖已记录的应用后 diff 元数据。** 缺少此元数据的通用编辑位置和终端命令不提供行数总计。
