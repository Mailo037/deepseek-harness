...
Produced-files, clickable-reference, and line-change-summary feature owner. The Node half registers final-response guidance with the system-prompt registry; the browser half registers the deliverables row a finished turn ends with into the chat view's `conversation.chat.turnTail` hole, the centered change-total disclosure into `conversation.input.dock`, and matching inline-code references in closing prose. The shipped Web patch is the only composition that loads this package. Removing its one cordis.yml entry removes the guidance, both UI readouts, and prose links together.
...
`LineChangeSummary` reads the same Turn data at the Composer's input dock. It admits only successful result-time diff cards, because their hunks are the applied change rather than the call-time intent. It groups paths in Turn order over the assembled Session timeline, sums repeated paths, and displays the file count with green additions and red removals. Clicking the centered total turns its chevron upward and opens a scrollable list with each filename first, its full path after a middle dot, and counts on the right; Escape or a click outside closes it. The disclosure is presentation-local: it adds no Session event, Host request, or model input.
...
- **The Composer change summary covers only applied diff cards in the assembled timeline.** Generic edit locations lack exact hunk counts, and history that the browser has not reconstructed cannot be represented as an exact total.
