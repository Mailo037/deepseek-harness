...
`LineChangeSummary` reads the whole-session `sessionStats.fileChanges` projection at the Composer's input dock. The first history page supplies compact per-path totals, and projection updates refresh them as tools finish; older messages and diff bodies need not be loaded. Repeated paths accumulate additions and removals. Clicking the centered total opens a scrollable list with each filename, full path, and counts; Escape or a click outside closes it. Assemblies without the projection or with a received projection lacking its per-file breakdown use the loaded Turn data as a fallback until complete statistics arrive. The disclosure adds no Session event or model input.
...
- **The Composer change summary requires recorded applied diff metadata.** Generic edit locations and terminal commands without that metadata provide no line totals.
