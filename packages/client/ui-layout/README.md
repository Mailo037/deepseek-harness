...
When `ctx.connection.shell` reports the Android shell, AppFrame keeps the current content visible through connection recovery and suppresses the ordinary full-screen connection-lost overlay. The connection service reports its state to the parent shell, which owns the single inline Android status; ordinary browser tabs retain the delayed overlay.
