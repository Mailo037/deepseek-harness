...
The cards this package ships cover the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and web search: which provider answers (`web-search` selection namespace) plus the DeepSeek provider's endpoint and budget (`web-search-deepseek`).
