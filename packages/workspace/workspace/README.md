...
- `ctx.workspaceRegistry.archiveSession(id)` / `unarchiveSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot. Restore removes the id without changing that slot, so the session returns to its prior position; both operations are idempotent. Archiving accepts any live or persisted session (accounted or Ungrouped), and rejects an unknown id. State written before the field existed parses with an empty set.
- `ctx.workspaceRegistry.setSessionPinned(id, pinned)` / `pinnedSessionIds` — the registry-global ordered pin set. Pinning accepts any known, non-archived session and appends a new member in pin order; unpinning is idempotent. Archiving atomically removes the same id from this set, and stored state rejects duplicate ids or archive/pin overlap. State written before the field existed parses with an empty set.
