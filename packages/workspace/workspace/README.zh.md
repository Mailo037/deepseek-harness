...
- `ctx.workspaceRegistry.archiveSession(id)`/`unarchiveSession(id)`/`archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合：被归档的会话从各分组视图中消失，但其会话日志和 `sessionIds` 席位保持不变。恢复只移除该 id 而不改变席位，因此会话回到原位置；两个操作均为幂等。归档接受任何实时或已持久化的会话（无论已记账还是 Ungrouped），并拒绝未知 id。在该字段出现之前写入的状态解析为一个空集合。
- `ctx.workspaceRegistry.setSessionPinned(id, pinned)`/`pinnedSessionIds`：注册表级全局有序固定集合。固定接受任何已知且未归档的会话，并按固定顺序追加新成员；取消固定是幂等的。归档会以原子方式从该集合移除同一 id，持久状态会拒绝重复 id 以及归档/固定集合重叠。在该字段出现之前写入的状态解析为一个空集合。
