...
更新平面同样经由 host 领域。`host.describe` 携带本次安装的 `version`（运行中安装里最近的 manifest）、其背后的 git `repository` 身份（分支/提交/远程 URL，非检出处为 null）、`canRestart`（组合中的启动器是否提供 `ctx.appRestart`），以及承载本 API 的 `surface`（`web` 或 `electron`）。`host.checkUpdate` 将本地 HEAD 与当前分支的上游比较——github.com 远程走一次公开的 GitHub Compare-API 请求，其余回退为 `git fetch`——并带短促的服务端缓存；它与 `host.applyUpdate` 都被钉在环回上（check 会让宿主发起网络请求；apply 会为每个客户端终结进程）。apply 先静默所有活跃 agent（保留收件箱工作的取消加有界排空），只做快进合并、应答，并在片刻之后调度启动器重生；分叉的树以 `self-update-not-fast-forward` 拒绝而不改写历史。这些方法背后的服务见 [dsh-host-self-update](../self-update/README.zh.md)。
