...
Both modes use bounded exponential backoff with symmetric jitter. A `RATE_LIMIT` failure (the adapters' classification of HTTP 429) scales its locally computed delay by the backoff's `rateLimitMultiplier` (default 2), so a rate-limited request waits longer before the next retry than a server or transport hiccup; the effective delay stays capped by `maxDelayMs`. A valid `providerRetryAfterMs` at or below `maxDelayMs` replaces local backoff verbatim and is never scaled, because the provider owns that wait. An over-cap provider delay makes normal mode delegate, while always mode uses its configured local backoff so it cannot terminate on that instruction.
