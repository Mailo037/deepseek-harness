...
两种 mode 都使用带对称 jitter 的有界指数退避。`RATE_LIMIT` 失败（适配器对 HTTP 429 的分类）会按其本地计算延迟乘上退避配置中的 `rateLimitMultiplier`（默认 2），因此受速率限制的请求在下次重试前会比服务器或传输故障等待更久；有效延迟仍受 `maxDelayMs` 上限约束。有效 `providerRetryAfterMs` 不超过 `maxDelayMs` 时会原样替换本地退避，且绝不缩放，因为该等待由提供方决定。超出上限的提供方延迟会使 normal mode 继续委托；always mode 则改用已配置的本地退避，避免该指令终止重试。
