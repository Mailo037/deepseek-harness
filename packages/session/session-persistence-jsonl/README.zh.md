...
- **崩溃恢复：保留有效尾部工作。**`load` 验证每个完整压缩 frame，并扫描解压 JSONL。最后 frame 结构不完整时，读取器保留其完整解码记录，从 frame 开头截断，并使用共享[持久化约定](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.zh.md) 需要的合成工具、步骤和轮次 closer 重新编码这些记录。原始 mode 从第一个不完整行截断。重复 seq 会被拒绝；唯一例外是陈旧 `session/end-seed` 标记紧邻具有相同 seq 的活动生命周期事件之前，此时读取器只丢弃该纯日志标记，并保留连续生命周期。已经存在却没有完整 header frame 的压缩产物、完整 frame 中的 checksum／解压失败，或位于最后已提交的 `turn/end` 处或之前的任何其他缺陷都属于损坏，会被拒绝。[冲突恢复决策](../../../.agents/notes/implemented/bug-fix/2026-08-24-jsonl-stale-end-seed-collision-recovery.zh.md)定义了这项狭窄例外。
