...
- **Crash recovery — preserve valid tail work.** `load` validates every complete compressed frame and scans their decompressed JSONL. If the last frame is structurally incomplete, the reader keeps its complete decoded records, truncates from that frame's start, and re-encodes those records with the synthetic tool, step, and turn closers required by the shared [persistence contract](../../../.agents/notes/implemented/architecture/2026-06-14-session-persistence.md). Raw mode truncates from its first incomplete line. A duplicate seq rejects except when a stale `session/end-seed` marker immediately precedes the active lifecycle event with the same seq; the reader drops only that log-only marker and retains the contiguous lifecycle. An existing compressed artifact with no complete header frame, a checksum/decompression failure in a complete frame, or any other defect at or before the last committed `turn/end` is corruption and rejects. The [collision-recovery decision](../../../.agents/notes/implemented/bug-fix/2026-08-24-jsonl-stale-end-seed-collision-recovery.md) defines the narrow exception.
