...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    keyDomain: 'fixed by the owner\'s key table { [Kind in ChatNodeKind]: { node: ChatNode<Kind> } }, already taken: assistant-step, command, command-input, compaction, context, manual-compaction, model-retry, shell-command, steering, tool-call, turn-error, turn-max-tokens, turn-tail, unknown, user, workflow-run',
    hookContext: 'string',
    ...
    occupants: [
      ...
      'client-ui-shell-command ShellCommandCard key \'shell-command\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-deliverables LineChangeSummary id \'line-changes\'',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation.composer.bar\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-voice-input VoiceInput',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.input.voice\', () => ctx.slots.register(\n      { name: \'conversation.input.voice\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-conversation/src/client/contract/slots.ts:299',
  },
  {
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation.session.header\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-workspace SessionOptionsAction id \'session-options\'',
      'session-log-export SessionLogDownloadDialog id \'session-log-download\'',
    ],
    replaceRisk: 'none',
  ...
  {
    key: 'conversation.session.header.utilities.menuHead',
    kind: 'single',
    scope: 'session',
    summary: 'Pinned context above the more-options menu\'s verb rows, declared by the SessionOptionsAction entry.',
    doc: 'Pinned context above the more-options menu\'s verb rows, declared by\nthe SessionOptionsAction entry. The owner passes nothing: an occupant\nreports read-only session facts (the running agent preset, for\nexample) and owns its own responsive visibility.',
    registerOptions: [],
    ownerProps: [],
    ownerPropsReferences: [],
    standardProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.session.header.utilities\' (client-ui-workspace), so it exists while that entry is mounted',
    occupants: [
      'client-ui-agent-preset AgentPresetMenuHead',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.session.header.utilities.menuHead\', () => ctx.slots.register(\n      { name: \'conversation.session.header.utilities.menuHead\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-remote RemoteSettingsSection id \'remote\'',
      'client-ui-remote FsDenySection id \'fs-deny\'',
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'root\' (client-ui-layout), so it exists while that entry is mounted',
    occupants: [
      'client-ui-settings-general ApplyingUpdateOverlay id \'self-update-progress\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    source: 'packages/client/ui-layout/src/client/index.ts:59',
  },
  {
