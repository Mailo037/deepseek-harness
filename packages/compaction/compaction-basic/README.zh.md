...
- **路由策略**：主动压力从拥有最新持久提供方／模型路由的适配器解析容量，再将默认策略与可选的精确目标覆盖缩放为具体 token 预算。模型发现仍仅供参考，不参与此处的策略解析。提供方已确认的溢出会先将其计量到的请求大小采纳为该聊天有效上下文窗口，并优先于已公布容量，使压力能对真实上限而非乐观值作出反应。
...
- **溢出恢复**：提供方已确认的溢出不需容量元数据。它会绕过常规压力与保留，执行剪枝，再尝试一次最大平衡头部缩减，并留下最新不可分单元。只要 `surface.replaceGeneration` 前进，就允许重试，包括剪枝在后续摘要工作抛出异常前已落地的情况。即使随后没有重试，也会将该计量到的请求大小采纳为该聊天有效上下文窗口，以便后续压力门槛依据真实上限工作。如果没有替换、目标特定上限已耗尽、已取消，或遇到未知／非规范错误，则保留原始提供方失败。
...
- **溢出分类由适配器维护**：提供方措辞可能改变；两个 DeepSeek 适配器将当前可识别的上下文限制失败规范化为 `CONTEXT_WINDOW_EXCEEDED`（包括诸如 `Input token exceed the limit` 的措辞）。
- **被采纳的窗口只约束压缩，不影响 request-context 计量**：被采纳的有效窗口只引导主动压力；agent loop 仍会以适配器公布的容量重新发出 `request/context`，因此 context-pressure 计量与 Web 占有率读数反映的是那个公布的窗口，而非被采纳的窗口。当一个部署的实际容量低于公布值且未让 loop 感知到被采纳窗口时，占有率会显示得偏保守（更低）。
