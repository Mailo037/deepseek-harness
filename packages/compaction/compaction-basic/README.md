...
- **Routed policy** — proactive pressure resolves capacity from the adapter that owns the latest durable provider/model route, then scales the default policy plus an optional exact-target override into concrete token budgets. Model discovery remains advisory and is not consulted. A provider-confirmed overflow first adopts its measured request size as the chat's effective window, which then takes precedence over the advertised capacity so pressure reacts to the true limit rather than an optimistic one.
...
- **Overflow recovery** — provider-confirmed overflow needs no capacity metadata: it bypasses normal pressure and retention, prunes, then attempts one maximal balanced head reduction while leaving the newest indivisible unit. Retry is authorized whenever `surface.replaceGeneration` advances, including when pruning lands before later summary work throws. The measured request size is adopted as the chat's effective context window even when no retry follows, so later pressure gating uses the real limit. No replacement, an exhausted target-specific cap, cancellation, or an unknown/noncanonical error preserves the original provider failure.
...
- **Overflow classification is adapter-maintained** — provider wording can change; both DeepSeek adapters normalize currently recognized context-limit failures to `CONTEXT_WINDOW_EXCEEDED` (including wording such as `Input token exceed the limit`).
- **The learned window gates compaction, not the request-context meter** — a learned effective window steers proactive pressure; the agent-loop still re-emits `request/context` from the adapter-advertised capacity, so the context-pressure meter and the Web occupancy readout reflect that advertised window rather than the learned one. A model whose deployment capacity is smaller than the advertised value shows a conservative (lower) occupancy until the loop is taught to consult the learned window.
